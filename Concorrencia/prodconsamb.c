...
pthread_cond_t  vazio1, cheio2,vBuffer3, cheio1, vazio2, cheioBuf3;
...
void *produtor(void *arg) {
   ...
   for (i = 0; i < loops; i++) {
      ...
      while (count1 == max) Pthread_cond_wait(&vazio1, &mutex1);
      produz(i,1);
      printf("Produtor %lld produziu %d em 1\n", (long long int) arg, i);
      Pthread_cond_signal(&cheio1);
...
void *ambos(void *arg) {
   ...
   for(i = 0 ; i < loops; i++){
      Pthread_mutex_lock(&mutex1);
      while (count1 == 0) Pthread_cond_wait(&cheio1, &mutex1);
      tmp = consome(1);
      printf("Ambos %lld consumiu %d em 1\n", (long long int) arg, tmp);
      Pthread_cond_signal(&vazio1);


      while(count2 == max) Pthread_cond_wait(&vazio2 ,&mutex1);
      produz(tmp,2);
      printf("Ambos %lld produziu %d em 2\n", (long long int) arg, tmp);
      Pthread_cond_signal(&cheio2);
      Pthread_mutex_unlock(&mutex1); 
   }
   return NULL;
}
